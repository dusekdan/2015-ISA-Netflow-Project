...
int main(int argc, char * argv[])
{
    ...
    /// Signal registration
    signal(SIGINT, &terminationCleanUp);
                    ...
                    case protoNumber_TCP: // TCP
                        ...
                        else
                        {
                            ...
                            if(createNewFlow)
                            {
                                ...
                                // Case when there is no cache allowed
                                if(Parameters.maxFlows == 0)
                                {
                                    createExpiredTCPFlow(sourceAddress, destinationAddress, sourcePort, destinationPort, protocolType, bytesTransfered, header.ts, prettySourceAddress, prettyDestinationAddress, tcpFlags, my_ip->ip_tos, my_tcp->th_flags);
                                }
                                else
                                {
...
/**
  * Creates TCPFlow with all its requisities, but is expired
  * @param  in_addr     sourceAddress               Source address of the flow
  * @param  in_addr     destinationAddress          Destination address of the flow
  * @param  u_short     sourcePort                  Source port of the flow
  * @param  u_short     destinationPort             Destination port of the flow
...
void createExpiredTCPFlow(in_addr sourceAddress, in_addr destinationAddress, u_short sourcePort, u_short destinationPort, u_int protocolType, u_int bytesTransfered, struct timeval startTime, const char* prettySourceAddress, const char* prettyDestinationAddress, bitset<8> tcpFlags, uint8_t typeOfService, uint8_t tcpFlagsForCollector)
{
    ...
    {
        expiredFlows.push_back(newFlow);      // Otherwise putting the flow to processedFlows (active)
    }   
...
/**
  * Signal handler functions - designed to handle CTRL+C interuption of the program. In that case expireFlows() and exportFlows() is called before exiting
  * @param  int     signal  Signal that caused this callback
  */
void terminationCleanUp(int signal)
{

    // The reality has shown that when program is terminated too soon (no expired flows are created), program gets stuck while trying to finish the job 
    // which happens to be unwanted behavior, plain information about program being stopped by signal will have to do
    cerr << endl;
    cerr << "Program terminated by CTRL+C, program will now do clean up and finish currently active tasks." << endl;
    cerr << "In very rare scenarios, it is possible for program to become stuck at this moment. " << endl;
    cerr << "It is typically caused by system-exception (too many signals flying around) and can not be handled properly." << endl;
    cerr << "If this is the case, I am extremely sorry for the inconveniece, you will have to kill me with \"kill -9 [PID]\"" << endl;
    cerr << "To find out [PID], use \"ps ux\" from another terminal." << endl;

    // Original Idea
    // Firstly expire flows that were in processedFlows vector
    expireFlows();

    // Secondly export those flows to collector
    exportFlows();

    // Now it is okay for program to be terminated
    exit(1);
}