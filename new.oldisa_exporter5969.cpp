				...
				switch(my_ip->ip_p)
				{
					...
					case 17: // UDP
						...
						// Get all 5 values to identify flow
							...
							// Initializations
							const char *prettyUDPSourceAddress, *prettyUDPDestinationAddress;
							in_addr sourceUDPAddress, destinationUDPAddress;
							u_short sourceUDPPort, destinationUDPPort;
							u_int bytesUDPTransfered;



							// Data load
							sourceUDPAddress		= (in_addr) my_ip->ip_src;
							destinationUDPAddress 	= my_ip->ip_dst; 
							sourceUDPPort 			= (u_short) my_udp->uh_sport;
							destinationUDPPort 		= (u_short) my_udp->uh_dport;
							protocolType			= 17;	// Given by case
							bytesUDPTransfered		= header.len;
						...
						// TESTING CORRECTNESS OF DATA
							...
							auxBuff = inet_ntoa(my_ip->ip_src);
							prettyUDPSourceAddress = strcpy(new char[strlen(auxBuff)+1], auxBuff);

							auxBuff = inet_ntoa(my_ip->ip_dst);
							prettyUDPDestinationAddress = strcpy(new char[strlen(auxBuff)+1], auxBuff);

							cout << "Source address: " << prettyUDPSourceAddress << " Destination address: " << prettyUDPDestinationAddress << endl;
							cout << "Source port: "	<<  sourceUDPPort	<< " Destination port: " << destinationUDPPort << endl;
					...
					// Check if flow exists
						...
						{
							// TODO: Replace this line with createFlow(sourceAddress, destinationAddress, sourcePort, destinationPort, protocolType, prettySourceAddress);
							createDebugFlow(sourceUDPAddress, destinationUDPAddress, sourceUDPPort, destinationUDPPort, protocolType, bytesUDPTransfered, prettyUDPSourceAddress, prettyUDPDestinationAddress);
						...
						else
						{
							...
							// Initialize on true, set to false if flow exists (and is discovered by for loop)
							bool createNewUDPFlow = true;

							// Iterating through vector of flows
							for(vector<T_Flows>::iterator it = processedFlows.begin(); it != processedFlows.end(); ++it)
							{

								if(it->sourceAddress.s_addr == sourceUDPAddress.s_addr && it->destinationAddress.s_addr == destinationUDPAddress.s_addr && it->sourcePort == sourceUDPPort && it->destinationPort == destinationUDPPort && it->protocolType == protocolType)
								{
									// TRUE: Update metrics in the flow 
									// TODO: DO THAT
									cout << "JUST RUN OVER THE EXISTING FLOW (S: " << it->prettySourceAddress << ":" << it->sourcePort <<", D:" << it->prettyDestinationAddress << ":"<< it->destinationPort <<") (current processedflows: " << processedFlows.size() << ")"  << endl;
							...
							if(createNewUDPFlow)
							{
									...
									createNewUDPFlow = false;
									// TODO: Debug only, remove this
									cout << "ADDED NEW FLOW TO processedFlows (S: " << prettyUDPSourceAddress << ":" << sourceUDPPort <<", D:" << prettyUDPDestinationAddress << ":"<< destinationUDPPort <<") (current processedflows: " << processedFlows.size() << ")" << endl;
							}
						}
					break;
